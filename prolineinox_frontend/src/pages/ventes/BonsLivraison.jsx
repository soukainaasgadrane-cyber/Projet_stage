...
export default function BonsLivraison() {
  ...
  const fetchDocuments = async () => {
    setLoading(true);
    try {
      const res = await getDocuments({ type: 'delivery_note', search });
  ...
  const handlePDF = (id) => {
    downloadPDF(getDocumentPDF(id, 'delivery_note'), `delivery_note_${id}.pdf`);
  };
    ...
    { header: 'Statut', accessor: 'status' },
    {
      ...
      accessor: (row) => (
        <div className="flex gap-2">
          <Link to={`/ventes/bons-livraison/${row.id}`} className="inline-flex items-center gap-1 rounded-md border border-blue-200 px-2 py-1 text-sm font-medium text-blue-700 hover:bg-blue-50"><PencilIcon className="h-4 w-4" /> Modifier</Link>
  ...
  return (
    <div className="bg-white rounded shadow">
      <div className="p-4 border-b flex justify-between items-center">
        <h2 className="text-xl font-semibold">Bons de livraison</h2>
        <Link to="/ventes/bons-livraison/nouveau" className="bg-indigo-600 text-white px-4 py-2 rounded flex items-center gap-2">
