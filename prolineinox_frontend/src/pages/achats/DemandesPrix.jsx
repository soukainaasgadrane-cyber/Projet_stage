...
export default function DemandesPrix() {
  ...
  const fetchData = async () => {
    try {
      const res = await api.get('/purchase-requests', { params: { search } });
    ...
    { header: 'Statut', accessor: 'status' },
    {
      ...
      accessor: (row) => (
        <div className="flex gap-2">
          <Link to={`/achats/demandes-prix/${row.id}`} className="text-blue-600"><EyeIcon className="w-5 h-5" /></Link>
  ...
  return (
    <div className="bg-white rounded shadow">
      <div className="p-4 border-b flex justify-between">
        ...
        <Link to="/achats/demandes-prix/nouveau" className="bg-indigo-600 text-white px-4 py-2 rounded flex items-center gap-2">
          <PlusIcon className="w-5 h-5" /> Nouvelle demande
        </Link>
      </div>
      <div className="p-4">
