...
export default function BonReception() {
  ...
  const fetchData = async () => {
    try {
      const res = await api.get('/receipts-notes', { params: { search } });
    ...
    { header: 'Statut', accessor: 'status' },
    {
      ...
      accessor: (row) => (
        <div className="flex gap-2">
          <Link to={`/achats/bons-reception/${row.id}`} className="text-blue-600"><EyeIcon className="w-5 h-5" /></Link>
  ...
  return (
    <div className="bg-white rounded shadow">
      <div className="p-4 border-b flex justify-between">
        <h2 className="text-xl font-semibold">Bons de réception</h2>
        <Link to="/achats/bons-reception/nouveau" className="bg-indigo-600 text-white px-4 py-2 rounded flex items-center gap-2">
