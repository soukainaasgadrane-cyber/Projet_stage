...
export default function BonReception() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await getDocuments({ type: 'receipt_note', search });
  ...
  const handlePDF = async (row) => {
    try {
      await downloadPDF(getDocumentPDF(row.id, row.type), `bon_reception_${row.id}.pdf`);
    } catch (err) {
    ...
    { header: 'Statut', accessor: 'status' },
    {
      ...
      accessor: (row) => (
        <div className="flex gap-2">
          <Link to={`/achats/bons-reception/${row.id}`} className="text-blue-600" title="Consulter">
  ...
  return (
    <div className="rounded bg-white shadow">
      <div className="flex items-center justify-between border-b p-4">
        <h2 className="text-xl font-semibold">Bons de reception</h2>
        <Link to="/achats/bons-reception/nouveau" className="flex items-center gap-2 rounded bg-indigo-600 px-4 py-2 text-white">
